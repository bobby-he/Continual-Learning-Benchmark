...
from utils.metric import ip, _two_by_two_solve, _eval_quadratic_no_c
...
class L2(NormalNN):
    ...
    def optimizer_step(self):
        ...
        grad_list = [-1 * params[i].grad for i in range(len(params))]
        
        dummy = torch.zeros_like(self.output_logits['All'])
        dummy.requires_grad = True
        if self.gpu:
            dummy = dummy.cuda()
        
        g = torch.autograd.grad(self.output_logits['All'], self.model.parameters(), grad_outputs = dummy, create_graph = True)
        jvp_precon = torch.autograd.grad(g, dummy, grad_outputs = grad_list, retain_graph = True)
        with torch.no_grad():
            mft_precon_temp = self.mft_sqrt_factor * jvp_precon[0]
            mft_precon = mft_precon_temp - self.output_probs * torch.sum(mft_precon_temp, dim = 1, keepdim = True)
            
            m_11 = torch.sum(mft_precon * mft_precon) / self.batch_size
            c_1 = -1. * ip(grad_list, grad_list)
            b_11 = -1. * self.config['reg_coef'] * c_1
        ...
        if self.first_update:
            ...
            with torch.no_grad():
                b_21 = self.config['reg_coef'] * ip(grad_list, self.optimizer.prev_update_list)
                b_22 = self.config['reg_coef'] * ip(self.optimizer.prev_update_list, self.optimizer.prev_update_list)
                
                mft_prev_temp = self.mft_sqrt_factor * jvp_prev[0]
                mft_prev = mft_prev_temp - self.output_probs * torch.sum(mft_prev_temp, dim = 1, keepdim = True)
                
            m_21 = torch.sum(mft_precon * mft_prev) / self.batch_size
            m_22 = torch.sum(mft_prev * mft_prev) / self.batch_size
            
            c_2 = -1. * b_21 / self.config['reg_coef']
    ...
    def update_model(self, inputs, targets, tasks):
        ...
        self.output_logits = self.forward(inputs)
        with torch.no_grad():
            self.output_probs = self.sig_fn(self.output_logits['All'])
            self.mft_sqrt_factor = torch.sqrt(self.output_probs)
            
        loss = self.criterion(self.output_logits, targets, tasks)
        self.optimizer.zero_grad()
        loss.backward(retain_graph = True)
        self.optimizer_step()
        
        return loss.detach(), self.output_logits
...
class SI(L2):
    ...
    def update_model(self, inputs, targets, tasks):
        ...
        loss = self.criterion(self.output_logits, targets, tasks, regularization=False)
        self.optimizer.zero_grad()
        loss.backward(retain_graph=True)
        for n, p in self.params.items():
        ...
        loss = self.criterion(self.output_logits, targets, tasks, regularization=True)
        self.optimizer.zero_grad()
        loss.backward(retain_graph = True)
        self.optimizer_step()
